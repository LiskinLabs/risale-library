...
`schemaVersion` identifies the raw `config.json` schema written by Risale AI Studio.
Current version:
...
overrides and are merged with defaults when Risale AI Studio loads the config.
...
For notes in `booknotes`, Risale AI Studio stores note ranges with:
