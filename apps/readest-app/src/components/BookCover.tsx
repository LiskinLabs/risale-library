...
const BookCover: React.FC<BookCoverProps> = memo<BookCoverProps>(
  ({
    ...
    return (
      ...
      >
        <div className={clsx('book-3d-object relative h-full w-full', mode === 'grid' && 'premium-transition hover-glow')}>
          <div className={clsx('absolute inset-0 z-20 overflow-hidden bg-[#0c1222]', mode === 'grid' ? 'border-premium-gold/20 rounded-r-lg border-l-[4px]' : 'rounded-lg')}>
            {coverFit === 'crop' ? (
              <Image
