...
On-device analysis (dev build `com.liskinlabs.risale-ai-studio`, pulled via `xcrun devicectl device copy from --domain-type appDataContainer`):

- The `'Cache'` base = Tauri `appCacheDir()` = **`Library/Caches/com.liskinlabs.risale-ai-studio`** only — NOT all of `Library/Caches`. On the test device this held ~272 MB: a 249 MB duplicate dictionary (`concise-enhanced.mdx`, canonical copy lives in `App Support/Risale AI Studio/Dictionaries/<id>/`), ~23 MB duplicate import-staged epubs (canonical in `App Support/Risale AI Studio/Books/<hash>/`), the `search/` results cache, plus tiny system scratch. All safe to clear — every large item is an orphaned import/download staging duplicate.
...
- Never clear `Library/Application Support/com.liskinlabs.risale-ai-studio/Risale AI Studio` — the real Books/Dictionaries/Fonts/DB (~1.7 GB).
