...
export const ReaderHeader: React.FC<ReaderHeaderProps> = ({ title, backUrl = '/' }) => {
  ...
  return (
    <header
      className={`reader-header ${!isVisible ? 'pointer-events-none -translate-y-full opacity-0' : ''}`}
    >
      ...
      >
        ...
        <button
          ...
          onClick={(e) => {
            e.stopPropagation();
            isSettingsPanelOpen.set(!isSettingsPanelOpen.get());
          }}
        ...
        <div style={{ position: 'relative' }}>
          ...
          {isDownloadOpen && (
            <>
              ...
              />
              <div
                className='glass-panel absolute right-0 z-50 mt-2 w-40 overflow-hidden rounded-xl shadow-2xl'
                style={{ top: '100%', display: 'flex', flexDirection: 'column' }}
              >
                {['EPUB', 'FB2', 'PDF', 'MOBI'].map((format) => (
                  <button
                    key={format}
                    className='px-4 py-2 text-left text-sm transition-colors hover:bg-black/5 dark:hover:bg-white/5'
                    onClick={() => {
