...
export const ProgressFooter: React.FC<ProgressFooterProps> = ({ chapterName = '' }) => {
  ...
  useEffect(() => {
    ...
    if (containerScroll)
      containerScroll.addEventListener('scroll', updateProgress, { passive: true });
  ...
  useEffect(() => {
    ...
    if (minutes < 1) {
      setTimeEstimate('менее 1 мин');
      return;
    }
    if (minutes < 60) {
      setTimeEstimate(`~${minutes} мин до конца`);
      return;
    }
  ...
  return (
    <footer className='reader-footer'>
      ...
      <div className='reader-footer-info'>
        ...
        <span>
          {pageInfo ? `Стр. ${pageInfo} · ` : ''}Прочитано {progress}% · {timeEstimate}
        </span>
      </div>
    </footer>
  );
